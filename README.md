...
git init
git add .
git commit -m "Prepare Pizza POS for GitHub"
```

## Scripts

```bash
npm run dev            # backend + frontend for browser development
npm run dev:desktop    # Electron desktop app with Vite frontend
npm run build          # frontend production build
npm run desktop        # run Electron against the built frontend
npm run build:desktop  # build frontend and create installer
npm start              # backend API only
```

## License

Set your preferred license before publishing. MIT is a common choice for open-source projects.
